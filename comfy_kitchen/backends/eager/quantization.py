...
def grouped_int8_convrot_linear_packed(
    ...
    """Apply ConvRot INT8 linear to packed, expert-sorted activation rows."""
...
def grouped_convrot_w4a8_linear_packed(
    ...
    """Apply ConvRot W4A8 linear to packed, expert-sorted activation rows."""
    from comfy_kitchen.backends.eager.convrot_w4a4 import convrot_w4a4_linear
    ...
    if offsets[0] != 0 or offsets[-1] != x.shape[0] or any(
        ...
        raise ValueError("Packed grouped W4A8 ConvRot indptr must be monotonic from 0 through R")
    outputs = [
        convrot_w4a4_linear(
            x[offsets[expert]:offsets[expert + 1]],
            weight[expert],
...
@torch.library.custom_op("comfy_kitchen::grouped_int8_convrot_linear_packed", mutates_args=())
def _op_grouped_int8_convrot_linear_packed(
    x: torch.Tensor,
    expert_indptr: torch.Tensor,
    weight: torch.Tensor,
    weight_scale: torch.Tensor,
    ...
    return registry.get_implementation("grouped_int8_convrot_linear_packed", kwargs=kwargs)(**kwargs)
...
@torch.library.custom_op("comfy_kitchen::grouped_convrot_w4a8_linear_packed", mutates_args=())
def _op_grouped_convrot_w4a8_linear_packed(
    x: torch.Tensor,
    expert_indptr: torch.Tensor,
    weight: torch.Tensor,
    weight_scale: torch.Tensor,
    ...
    return registry.get_implementation("grouped_convrot_w4a8_linear_packed", kwargs=kwargs)(**kwargs)


@_op_grouped_convrot_w4a8_linear_packed.register_fake
def _op_grouped_convrot_w4a8_linear_packed_fake(
    x, expert_indptr, weight, weight_scale, convrot_groupsize, output_dtype_code
):
    del expert_indptr, weight_scale, convrot_groupsize
    return torch.empty(
