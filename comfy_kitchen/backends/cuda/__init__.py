...
def softcap_categorical_stats_sample_bf16(
    ...
    if (
        ...
        raise ValueError(
            "softcap_categorical_stats_sample_bf16 requires matching contiguous BF16/FP32 2D tensors"
        )
    if exponential_noise.device != raw_logits.device:
        raise ValueError("softcap_categorical_stats_sample_bf16 tensors must share one CUDA device")
    if raw_logits.shape[0] == 0 or raw_logits.shape[1] == 0:
    ...
    return self_conditioning_logits, entropy, argmax, sample, invalid
