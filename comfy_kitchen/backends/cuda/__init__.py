...
def restore_weighted_int8_moe_routes(
    ...
    """Restore expert-sorted rows and apply route weights into FP32 output."""
    if routed_output.dim() != 2 or routed_output.dtype not in (
        torch.float16,
        torch.bfloat16,
    ):
        raise ValueError("routed INT8 MoE output must be a CUDA FP16/BF16 matrix")
    if not routed_output.is_cuda:
        raise ValueError("routed INT8 MoE output must be on CUDA")
    routes = routed_output.shape[0]
    ...
    if (
        destinations.device != routed_output.device
        or weights.device != routed_output.device
    ):
        raise ValueError("INT8 MoE route tensors must share one CUDA device")
    destinations = (
        destinations if destinations.is_contiguous() else destinations.contiguous()
    )
    weights = weights if weights.is_contiguous() else weights.contiguous()
    output = torch.empty(
        routed_output.shape, dtype=torch.float32, device=routed_output.device
    )
    if not _C.restore_weighted_int8_moe_routes(
        ...
        raise RuntimeError("Native weighted INT8 MoE route restoration is unavailable")
    return output
