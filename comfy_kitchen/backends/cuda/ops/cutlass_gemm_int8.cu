...
__global__ void restore_weight_reduce_top8(
    ...
    __shared__ int64_t destinations[kTopK];
    __shared__ float weights[kTopK];
    if (threadIdx.x < kTopK) {
        const int route = token * kTopK + threadIdx.x;
    ...
    for (int col = threadIdx.x; col < hidden_size; col += blockDim.x) {
        float products[kTopK];
#pragma unroll
        for (int position = 0; position < kTopK; ++position) {
            const int64_t destination = destinations[position];
        ...
        output[static_cast<size_t>(token) * hidden_size + col] = from_float<T>(sum);
    }
}
}  // namespace int8_route_reduce

extern "C" bool launch_restore_weighted_reduce_int8_moe_routes(
    ...
    int dtype_code,
    cudaStream_t stream) {
    if (routes <= 0 || hidden_size <= 0 || top_k != int8_route_reduce::kTopK ||
        routes % top_k != 0 || routes / top_k > INT_MAX || hidden_size > INT_MAX) {
        return false;
    }
    const int tokens = static_cast<int>(routes / top_k);
    const int width = static_cast<int>(hidden_size);
    switch (dtype_code) {
        case 1:
            int8_route_reduce::restore_weight_reduce_top8<<<
                tokens, int8_route_reduce::kThreads, 0, stream>>>(
                static_cast<const __half*>(routed_output), route_dest, route_weights,
                static_cast<__half*>(output), width);
            break;
        case 2:
            int8_route_reduce::restore_weight_reduce_top8<<<
                tokens, int8_route_reduce::kThreads, 0, stream>>>(
                static_cast<const __nv_bfloat16*>(routed_output), route_dest, route_weights,
                static_cast<__nv_bfloat16*>(output), width);
            break;
        default:
            return false;
    }
