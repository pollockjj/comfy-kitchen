...
extern "C" void launch_gelu_tanh_multiply_quantize_int8_rowwise_convrot64_kernel(
    const void* gate,
    const void* up,
...
template <class T>
__global__ void restore_weight_reduce_top8(
    ...
    int hidden_size) {
    const int token = blockIdx.x;
    __shared__ int32_t destinations[kTopKMax];
    __shared__ float weights[kTopKMax];
    if (threadIdx.x < kTopKMax) {
        const int route = token * kTopKMax + threadIdx.x;
        destinations[threadIdx.x] = route_dest[route];
        weights[threadIdx.x] = route_weights[route];
    }
    __syncthreads();
...
#pragma unroll
        for (int position = 0; position < kTopKMax; ++position) {
            const int destination = destinations[position];
            products[position] = destination < 0
                ? __int_as_float(0x7fc00000)
                : __fmul_rn(
                    lowp_to_float(
                        routed_output[static_cast<size_t>(destination) * hidden_size + col]),
                    weights[position]);
        }
        const float pair0 = __fadd_rn(products[0], products[4]);
        const float pair1 = __fadd_rn(products[1], products[5]);
        const float pair2 = __fadd_rn(products[2], products[6]);
        const float pair3 = __fadd_rn(products[3], products[7]);
        float sum = __fadd_rn(pair0, pair1);
        sum = __fadd_rn(sum, pair2);
        sum = __fadd_rn(sum, pair3);
        output[static_cast<size_t>(token) * hidden_size + col] = float_to_lowp<T>(sum);
    }
}
...
bool run_fused_moe_int8_convrot(
    ...
    OutputType* intermediate = fuse_intermediate_quant
        ? nullptr
    ...
    if ((!fuse_intermediate_quant && intermediate == nullptr) || intermediate_qdata == nullptr ||
        intermediate_scales == nullptr) {
        return false;
    }
    try {
        if (fuse_intermediate_quant) {
            if (fc2_group_size != 64) {
                return false;
            }
            launch_gelu_tanh_multiply_quantize_int8_rowwise_convrot64_kernel(
                gate_up, gate_up + i, 2 * i, 2 * i, intermediate_qdata,
                intermediate_scales, routes, i, dtype_code, stream);
        } else {
    ...
    OutputType* stage2_intermediate = fuse_intermediate_quant
        ? nullptr
        : stage2.allocate<OutputType>(static_cast<size_t>(routes) * i);
    if ((!fuse_intermediate_quant && stage2_intermediate != intermediate) ||
    ...
    last_error_stage = 8;
    restore_weight_reduce_top8<OutputType><<<n, kRouteThreads, 0, stream>>>(
        routed_down, route_dest, router_weights, static_cast<OutputType*>(output), h);
    if (cudaPeekAtLastError() != cudaSuccess) {
        return false;
    }
    last_error_stage = 0;
...
extern "C" bool launch_cutlass_fused_moe_int8_convrot(
    ...
    if (dtype_code == 1) {
        return run_fused_moe_int8_convrot<__half>(
            ...
            fuse_intermediate_quant, workspace_ptr, static_cast<size_t>(workspace_size), stream);
    }
