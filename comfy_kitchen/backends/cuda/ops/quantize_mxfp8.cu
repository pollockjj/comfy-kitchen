...
#pragma unroll
        ...
        IType gate_vals[kMXFP8ValsPerThread];
        IType up_vals[kMXFP8ValsPerThread];
...
void launch_gelu_tanh_multiply_quantize_mxfp8_kernel(
    ...
    if (num_rows % comfy::kMXFP8BlockSize != 0 ||
        num_cols % comfy::kMXFP8BlockSize != 0) {
    ...
    DISPATCH_HALF_DTYPE(input_dtype_code, InputType, [&] {
        if (misaligned) {
            comfy::gelu_tanh_multiply_quantize_mxfp8_kernel<
                InputType, __nv_fp8_e4m3, true>
                <<<blocks, threads_per_block, 0, stream>>>(
