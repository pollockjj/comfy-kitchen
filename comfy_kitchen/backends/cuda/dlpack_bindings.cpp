...
void cutlass_fused_moe_int8_convrot(
    ...
    if (top_k != 8 || output.shape(0) != num_tokens || output.shape(1) != hidden_size ||
        map_dtype_to_code(output.dtype()) != input_dtype_code ||
    ...
    if (!launch_cutlass_fused_moe_int8_convrot(
            ...
            fc1_group_size, fc2_group_size, input_dtype_code, fuse_intermediate_quant,
            workspace.data(),
...
NB_MODULE(_C, m) {
    ...
    m.def("cutlass_fused_moe_int8_convrot", &cutlass_fused_moe_int8_convrot,
          ...
          nb::arg("fuse_intermediate_quant"),
          nb::arg("stream_ptr"));
