...
def test_packed_grouped_convrot_w4a4_matches_expert_rows(seed, backend):
    ...
    with ck.registry.use_backend(backend):
        ...
        actual = ck.grouped_convrot_w4a4_linear_packed(x, indptr, qweight, scales, 256)
        expected = torch.cat([
            convrot_w4a4_linear(x[start:end], qweight[expert], scales[expert])
